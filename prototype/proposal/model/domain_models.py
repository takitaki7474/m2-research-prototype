...
class LeNet3(nn.Module):
  ...
  def forward(self, x):
    x = F.relu(self.conv1(x)) # (1) 32*32*3 -> 32*32*16
    x = F.max_pool2d(x, 2, 2) # (2) 32*32*16 -> 16*16*16
    x = F.relu(self.conv2(x)) # (3) 16*16*16 -> 16*16*32
    x = F.max_pool2d(x, 2, 2) # (4) 16*16*32 -> 8*8*32
    x = F.relu(self.conv3(x)) # (5) 8*8*32 -> 8*8*64
...
class LeNet4(nn.Module):
  ...
  def forward(self, x):
    ...
    x = F.relu(self.conv3(x)) # (5) 8*8*32 -> 8*8*64
    x = F.relu(self.conv4(x)) # (6) 8*8*64 -> 8*8*128
    x = F.max_pool2d(x, 2, 2) # (7) 8*8*128-> 4*4*128
